...
let
  ...
  aurPackage =
    if !isPortableHost
    then null
    else pkgs.runCommand "waytorandr-aur-${workspace.version}" { } ''
      mkdir -p "$out"
  ...
  flatpakPackage =
    if !isPortableHost
    then null
    else pkgs.runCommand "waytorandr-flatpak-${workspace.version}" {
  ...
  snapPackage =
    if !isPortableHost
    then null
    else pkgs.runCommand "waytorandr-snap-${workspace.version}" {
